...
public class UserService extends BaseService<User,Integer> {
    ...
    private void checkPasswordParams(User user, String oldPwd, String newPwd, String repeatPwd) {
        ...
        //判断确认密码是董与新密码一致
        AssertUtil.isTrue(!newPwd.equals(repeatPwd),"确认密码与新密码不一致1");
    ...
    public void addUser(User user){
        /*1.参数校验*/
        checkUserParams(user.getUserName(),null);
     ...
     *      用户名userName     非空，唯一性
     */
    private void checkUserParams(String userName, Integer userId) {
        //判断用户名是否为空
        AssertUtil.isTrue(StringUtils.isBlank(userName),"用户名不能为空！");
        //判断用户名的一致性
        //通过用户名查询用户对象
        User temp = userMapper.queryUserByName(userName);
        //如果用户名对象为空 ，则表示用户名可用；如果用户名对象不为空，则表示用户名不可用
    ...
    public void updateUser(User user){
        ...
        //参数校验
        checkUserParams(user.getUserName(),user.getId());
